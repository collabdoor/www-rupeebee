...
function TipCard({ tip }: TipCardProps) {
  ...
  const badgeColor = isDo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700';
  const iconBgColor = isDo ? 'bg-green-100' : 'bg-red-100';
  const exampleColor = tip.example.type === 'warning' ? 'bg-red-50 border-red-400' : 
                      tip.example.type === 'success' ? 'bg-green-50 border-green-400' : 
                      'bg-yellow-50 border-yellow-400';
  const exampleTextColor = tip.example.type === 'warning' ? 'text-red-800' : 
                          tip.example.type === 'success' ? 'text-green-800' : 
                          'text-yellow-800';
  ...
  return (
    ...
    >
      <AccordionTrigger className={`w-full text-left p-6 ${hoverColor} transition-colors`}>
        <div className="flex items-center justify-between w-full">
          <div className="flex items-center gap-4">
            ...
            <div>
              <span className={`inline-block ${badgeColor} px-3 py-1 rounded-full text-sm font-medium mb-2`}>
                {isDo ? '✓ DO' : '✗ DON\'T'}
              </span>
      ...
      <AccordionContent>
        <div className="px-6 pb-6">
          ...
          {tip.details && (
            <p className="text-gray-600 mb-4">{tip.details}</p>
          )}
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="text-center max-w-4xl mx-auto mb-16 mt-32">
          ...
          <p className="text-xl text-gray-600 leading-relaxed">
            Master essential financial safety practices to protect yourself from fraud and build lasting wealth. 
            Your financial security starts with smart decisions and awareness.
          </p>
        </div>
          ...
          <div className="mt-16 text-center">
            <div className="bg-gradient-to-r from-green-600 to-green-700 text-white rounded-2xl p-8">
              ...
              <p className="text-green-100 mb-6">
                Download RupeeBee to get personalized financial safety tips and fraud alerts tailored just for you.
              </p>
