  ...
  const columns: ColumnDef<BankLearningModule>[] = [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            ...
            <div className="text-sm text-gray-500">{moduleData.content_type} • {moduleData.language}</div>
            <div className="text-xs text-gray-400 mt-1">
              Created: {new Date(moduleData.created_at).toLocaleDateString()}
            </div>
          </div>
        );
      },
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            <div className="text-sm text-gray-900">{moduleData.category}</div>
            <div className="text-xs text-gray-500">{moduleData.difficulty_level}</div>
          </div>
        );
      },
    },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="text-sm text-gray-500 space-y-1">
            <div>{moduleData.views_count} views</div>
            <div>{moduleData.completion_count} completions</div>
            <div className="flex items-center">
              <svg className="w-4 h-4 text-yellow-400 mr-1" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
              {moduleData.rating.toFixed(1)}
            </div>
          </div>
        );
      },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex space-x-2">
            <button
              onClick={() => handleTogglePublish(moduleData)}
              disabled={isUpdating === moduleData.id}
            ...
            >
              {isUpdating === moduleData.id ? 'Updating...' : (moduleData.is_published ? 'Unpublish' : 'Publish')}
            </button>
